...
#ifndef H_LCZ_FS_MGMT_IMPL_
#define H_LCZ_FS_MGMT_IMPL_
 ...
 * @return 0 on success, MGMT_ERR_[...] code on failure.
 */
int lcz_fs_mgmt_impl_read(const char *path, size_t offset, size_t len,
 ...
 * @return 0 on success, MGMT_ERR_[...] code on failure.
 */
int lcz_fs_mgmt_impl_write(const char *path, size_t offset, const void *data,
					   size_t len);

/**
 * @brief Computes a hash on a file
 *
 * @param path		The path of the file to hash.
 * @param size      Number of bytes to include in the hash calculation
 * @param output	On success, the hash
 *
 * @return 0 on success, MGMT_ERR_[...] code on failure.
 */
