...
#define CBOR_AND_OTHER_HDR \
	((9 + 1) + \
	 (1 + 3 + CONFIG_LCZ_FS_MGMT_MAX_OFFSET_LEN) + \
	 (1 + 4 + CONFIG_LCZ_FS_MGMT_MAX_OFFSET_LEN) + \
	 (1 + 2 + 1) + \
	 (1 + 3 + CONFIG_LCZ_FS_MGMT_MAX_OFFSET_LEN))

#if defined(CONFIG_LCZ_FS_MGMT_DL_CHUNK_SIZE_LIMIT)
#if (CONFIG_LCZ_FS_MGMT_DL_CHUNK_SIZE + CBOR_AND_OTHER_HDR) > MCUMGR_BUF_SIZE
#warning FS_MGMT_DL_CHUNK_SIZE too big, rounding it down.
#else
#define FS_MGMT_DL_CHUNK_SIZE (CONFIG_LCZ_FS_MGMT_DL_CHUNK_SIZE)
#endif
#endif
...
#define FS_MGMT_PATH_SIZE       CONFIG_LCZ_FS_MGMT_PATH_SIZE
#define FS_MGMT_UL_CHUNK_SIZE   CONFIG_LCZ_FS_MGMT_UL_CHUNK_SIZE
