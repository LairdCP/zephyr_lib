...
/******************************************************************************/
#define CONFIG_UART_MCUMGR_RX_BUF_SIZE CONFIG_DUMMY_SMP_RX_BUF_SIZE
...
static bool enable_dummy_smp = false;
static struct k_sem smp_data_ready_sem;
static uint8_t smp_send_buffer[CONFIG_UART_MCUMGR_RX_BUF_SIZE];
static uint16_t smp_send_pos = 0;
static uint8_t smp_receive_buffer[CONFIG_UART_MCUMGR_RX_BUF_SIZE];
static uint16_t smp_receive_pos = 0;
...
{
	return k_sem_take(&smp_data_ready_sem, K_SECONDS(wait_time_s));
}
